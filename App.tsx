...
const MODERATION_TRIGGER_REGEX = /\b(sexo|sexual|nude|nud3|nudez|porno|porn|nsfw|namora|namoro|ficar comigo|me ama|droga|drogas|coca[ií]na|maconha|crime|roubar|hackear|hack|phishing|golpe|carding|cpf|xingar|burro|idiota|ot[aá]rio|otario|spam)\b/i;

const shouldRunAiModeration = (text: string) => {
  const normalized = text.trim().toLowerCase();
  if (!normalized) return false;
  if (normalized.includes('sistem091p3919')) return true;
  return MODERATION_TRIGGER_REGEX.test(normalized);
};
...
🔥 Isso garante um código limpo, completo e sem erros.`,
  ...
  const handleSendMessage = useCallback(async (text: string, images?: string[], thinkingLevel?: string, useBlockMode?: boolean) => {
    ...
    try {
      ...
      } else if (shouldRunAiModeration(text)) {
        // Moderação local (sem IA) — preserva quota das chaves Gemini para a geração de código.
        const maskedEmail = (user.email || '').replace(/(.{2})(.*)(?=@)/, '$1***');
        const localReport = `[RELATÓRIO DE MODERAÇÃO LOCAL]\nUsuário: ${user.uid}\nEmail: ${maskedEmail}\nMensagem: "${text}"\nCategoria: SUSPEITO (palavra-chave detectada)\nRisco: médio\nAção: revisar manualmente\nResumo: Mensagem disparou filtro local de palavras-chave (sem análise por IA).`;
        addDoc(collection(db, 'moderation_reports'), {
          userId: user.uid,
          userEmail: user.email,
          chatId: chatId,
          ...
          createdAt: serverTimestamp(),
          resolved: false
        }).catch(e => console.error("Error saving local mod log", e));
      }
